...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            self.inputs['graph_idx'] = flat_inputs[4 * num_layers + 4]
    ...
    def inference(self, inputs, is_training):
        ...
        for j in range(self.config.num_layers):
            # Backward attentive fusing
            f_high_encoder = helper_tf_util.conv2d(f_encoder_list[-j - 1], f_encoder_list[-j - 2].get_shape()[-1].value,
                                                   [1, 1], 'Squeeze_layer_' + str(j), [1, 1], 'VALID', bn=True,
                                                   is_training=is_training)
            f_high_encoder_interp = self.nearest_interpolation(f_high_encoder, inputs['interp_idx'][-j - 1])
            attention_map = helper_tf_util.conv2d_transpose(f_high_encoder_interp,
                                                            f_encoder_list[-j - 2].get_shape()[-1].value, [1, 1],
                                                            'Attention_layer_' + str(j), [1, 1], 'VALID', bn=False,
                                                            is_training=is_training, activation_fn=None)
            attention_map = tf.nn.sigmoid(attention_map)

            f_fuse = f_encoder_list[-j - 2] * attention_map + f_encoder_list[-j - 2]

            f_enhance = helper_tf_util.conv2d(f_fuse, f_encoder_list[-j - 2].get_shape()[-1].value, [1, 1],
                                              'Enhance_layer_' + str(j), [1, 1], 'VALID', bn=True,
                                              is_training=is_training)

            # Decoder
            f_interp_i = self.nearest_interpolation(feature, inputs['interp_idx'][-j - 1])
            f_interp_i = helper_tf_util.conv2d_transpose(f_interp_i, f_encoder_list[-j - 2].get_shape()[-1].value,
                                                         [1, 1], 'Interp_layer_' + str(j), [1, 1], 'VALID', bn=True,
                                                         is_training=is_training)

            f_decoder_i = helper_tf_util.conv2d(f_interp_i + f_enhance, f_encoder_list[-j - 2].get_shape()[-1].value,
                                                [1, 1], 'Decoder_layer_' + str(j), [1, 1], 'VALID', bn=True,
                                                is_training=is_training)
            feature = f_decoder_i
        ...
        f_neigh = self.gather_neighbour(tf.squeeze(f_layer_fc2, axis=2), inputs['graph_idx'])
